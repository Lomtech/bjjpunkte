...
async function initSupabase() {
  try {
    ...
    if (res.ok) {
      ...
      if (supabaseUrl && supabaseAnonKey) { sb = createClient(supabaseUrl, supabaseAnonKey); return; }
    }
  } catch (_) {}
  if (LOCAL_SUPABASE_URL && LOCAL_SUPABASE_ANON_KEY) {
    sb = createClient(LOCAL_SUPABASE_URL, LOCAL_SUPABASE_ANON_KEY); return;
  }
...
const BELT_NAMES = { white:'Weißgurt', blue:'Blaugurt', purple:'Lillagurt', brown:'Braungurt', black:'Schwarzgurt' };
...
async function login() {
  ...
  if (!email || !password) return setLoginMsg('Bitte alle Felder ausfüllen.', 'error');
  setLoginMsg('…', '');
  const { data, error } = await sb.auth.signInWithPassword({ email, password });
  if (error) return setLoginMsg(error.message.includes('Invalid login') ? 'E-Mail oder Passwort falsch.' : error.message, 'error');
...
async function doLogin(user) {
  ...
  $('topbar-trainer').textContent = profile.full_name || user.email.split('@')[0];
...
function renderDashboard() {
  ...
  const todayActs = Object.values(activitiesMap).flat().filter(a => a.type === 'training' && a.date === TODAY).length;
  $('stat-today').textContent = todayActs;

  const beltReady = active.filter(a => getPoints(a.id) >= BELT_THRESHOLD && a.belt !== 'black');
  $('stat-belt-ready').textContent = beltReady.length;

  const pts = active.map(a => getPoints(a.id));
  $('stat-avg').textContent = pts.length ? Math.round(pts.reduce((s,p)=>s+p,0)/pts.length) : 0;

  // Belt alert
  const alert = $('belt-alert');
  if (beltReady.length) {
    alert.style.display = 'flex';
    $('ba-names').textContent = beltReady.map(a => `${a.name} → ${BELT_NAMES[BELT_NEXT[a.belt]]}`).join(', ');
  } else {
...
function renderAthletesList(filter = '') {
  ...
  if (!filtered.length) {
    list.innerHTML = '<div style="padding:24px 16px;text-align:center;color:var(--text3);font-size:14px">Keine Athleten gefunden</div>';
    return;
  }
  filtered.forEach(a => {
...
function renderEntryAthletes(filter = '') {
  ...
  if (selectedAthleteId) {
    ...
    $('selected-chip-name').textContent = a ? `${a.name} · ${BELT_NAMES[a.belt]}` : '';
    return;
  }
...
$('entry-search').addEventListener('input', e => renderEntryAthletes(e.target.value));
...
$('btn-submit-entry').addEventListener('click', async () => {
  if (!selectedAthleteId) return showEntryMsg('Bitte einen Athleten wählen.', 'error');
...
$('btn-quick-group').addEventListener('click', async () => {
  ...
  const rows = active.map(a => ({
    athlete_id: a.id, type: 'training', points: 1,
    date: TODAY, note: 'Gruppen-Training', created_by: currentUser.id,
  }));
...
document.querySelectorAll('#belt-picker .bg-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('#belt-picker .bg-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
...
document.querySelectorAll('#promote-picker .bg-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('#promote-picker .bg-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
...
function renderBeltAthleteSelect() {
  ...
  const val = sel.value;
  sel.innerHTML = '<option value="">— Athlet wählen —</option>' +
...
$('btn-belt-promote').addEventListener('click', async () => {
  ...
  const { error } = await sb.from('athletes').update({ belt: selectedPromoteBelt }).eq('id', id);
  if (error) return toast('Fehler: ' + error.message, 'err');
...
// Search athletes
$('search-athletes').addEventListener('input', e => renderAthletesList(e.target.value));
...
function openModal(athleteId) {
  ...
  grid.innerHTML = BELT_ORDER.filter(b => b !== a.belt).map(b => `
  ...
  grid.querySelectorAll('.bg-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      grid.querySelectorAll('.bg-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
  ...
  $('btn-deactivate').textContent = a.active === false ? 'Athlet reaktivieren' : 'Athlet deaktivieren';
...
async function quickEntry(athleteId, type) {
  ...
  btn.addEventListener('click', async () => {
    ...
    const { error } = await sb.from('activities').insert({
      athlete_id: openAthleteId, type: btn.dataset.type, points: t.pts,
      date: TODAY, note: t.label, created_by: currentUser.id,
    });
...
$('modal-belt-btn').addEventListener('click', async () => {
  if (!openAthleteId) return;
  const { error } = await sb.from('athletes').update({ belt: selectedModalBelt }).eq('id', openAthleteId);
  if (error) return toast('Fehler', 'err');
  const a = athletes.find(x => x.id === openAthleteId);
...
$('btn-deactivate').addEventListener('click', async () => {
  ...
  await sb.from('athletes').update({ active: newActive }).eq('id', openAthleteId);
...
function showScreen(name) {
  ...
  const s = $(`screen-${name}`);
  if (s) { s.classList.add('active'); if (name === 'app') s.style.display = 'flex'; }
}
