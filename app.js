...
async function initSupabase() {
  try {
    ...
    if (!res.ok) throw new Error('Config-Endpoint nicht erreichbar');
    const { supabaseUrl, supabaseAnonKey } = await res.json();
    if (!supabaseUrl || !supabaseAnonKey) throw new Error('Supabase-Credentials fehlen in den Netlify Env Vars');
    sb = createClient(supabaseUrl, supabaseAnonKey);
  } catch (err) {
    console.error('Supabase Init Fehler:', err);
    document.getElementById('auth-message').textContent = '⚠ Konfigurationsfehler: ' + err.message;
...
document.getElementById('btn-login').addEventListener('click', async () => {
  ...
  if (!email || !password) return setAuthMessage('Bitte alle Felder ausfüllen.', 'error');
...
document.getElementById('btn-register').addEventListener('click', async () => {
  ...
  if (!name || !email || !password) return setAuthMessage('Bitte alle Felder ausfüllen.', 'error');
  if (password.length < 6) return setAuthMessage('Passwort mindestens 6 Zeichen.', 'error');
...
document.getElementById('btn-add-tournament').addEventListener('click', async () => {
  await addActivity('tournament', TOURNAMENT_POINTS);
});

document.getElementById('btn-undo').addEventListener('click', async () => {
  if (!lastActivityId) return;
  const { error } = await sb.from('activities').delete().eq('id', lastActivityId);
  if (!error) {
...
function renderAll() {
  ...
  const year = new Date().getFullYear();
  const totalPoints = currentActivities.reduce((s, a) => s + (a.points || 0), 0);
  const trainingCount = currentActivities.filter(a => a.type === 'training').length;
  const tournamentCount = currentActivities.filter(a => a.type === 'tournament').length;

  // Header
  document.getElementById('header-username').textContent = currentProfile.full_name;
  ...
  document.getElementById('progress-label').textContent = Math.round(pct) + '% abgeschlossen';
  const sessionsLeft = Math.max(0, MAX_POINTS - trainingCount);
  document.getElementById('sessions-left').textContent = sessionsLeft + ' Trainings übrig';
  ...
  if (belt.next) {
    document.getElementById('belt-next-info').textContent = `→ Nächster Gürtel: ${belt.next} (${BELT_THRESHOLD} Punkte)`;
  } else {
    document.getElementById('belt-next-info').textContent = 'Höchster Gürtel erreicht 🏆';
  }
  ...
  const weekCount = currentActivities.filter(a =>
    a.type === 'training' && new Date(a.created_at) >= weekStart
  ).length;
  document.getElementById('stat-week').textContent = weekCount;

  // Streak (vereinfacht: Wochen mit mind. 1 Training in Folge)
  document.getElementById('stat-streak').textContent = calcStreak(currentActivities);
...
function renderHistory() {
  ...
  document.getElementById('history-count').textContent = currentActivities.length;

  if (currentActivities.length === 0) {
    list.innerHTML = '<div class="empty-state">Noch keine Einträge – fang mit dem nächsten Training an!</div>';
    return;
  }

  list.innerHTML = currentActivities.slice(0, 30).map(a => {
